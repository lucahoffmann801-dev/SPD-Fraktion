...
import { signUploadToken, isUploadEnabled, UPLOAD_ALLOWED_SLUGS } from "@/server/uploadAuth";
...
export async function POST(request: NextRequest) {
  const body = await request.json() as { profileSlug?: string; code?: string };
  const { profileSlug, code } = body;
  ...
  if (!profile) return NextResponse.json({ error: "Profil nicht gefunden oder deaktiviert." }, { status: 404 });

  const sharedCode = process.env.PORTAL_SHARED_CODE;
  if (sharedCode && code !== sharedCode) return NextResponse.json({ error: "Zugangscode stimmt nicht." }, { status: 403 });
  ...
  return NextResponse.json({ ok: true, profile, prototypeMode: !sharedCode, ...(uploadToken ? { uploadToken } : {}) });
}
