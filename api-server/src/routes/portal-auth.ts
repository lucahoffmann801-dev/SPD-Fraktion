...
router.get("/auth/profiles", async (_req, res) => {
  const supabase = getSupabase();
  if (!supabase) return res.json({ profiles: demoProfiles });
  const { data, error } = await supabase.from("profiles").select("*").eq("login_enabled", true).order("sort_order", { ascending: true });
  if (error) return res.json({ profiles: demoProfiles });
  return res.json({ profiles: data ?? demoProfiles });
});

router.post("/auth/session", async (req, res) => {
  const { profileSlug, code } = req.body as { profileSlug?: string; code?: string };
  const supabase = getSupabase();
  let profiles = demoProfiles;
  if (supabase) {
    const { data } = await supabase.from("profiles").select("*").eq("login_enabled", true).order("sort_order", { ascending: true });
    if (data) profiles = data as typeof demoProfiles;
  }
  const profile = profiles.find((p) => p.slug === profileSlug && p.login_enabled);
  if (!profile) return res.status(404).json({ error: "Profil nicht gefunden oder deaktiviert." });

  const sharedCode = process.env.PORTAL_SHARED_CODE;
  if (sharedCode && code !== sharedCode) return res.status(403).json({ error: "Zugangscode stimmt nicht." });

  const canUpload = isUploadEnabled() && UPLOAD_ALLOWED_SLUGS.includes(profile.slug as (typeof UPLOAD_ALLOWED_SLUGS)[number]);
  const uploadToken = canUpload ? signUploadToken(profile.slug) : undefined;
  return res.json({ ok: true, profile, prototypeMode: !sharedCode, ...(uploadToken ? { uploadToken } : {}) });
});
