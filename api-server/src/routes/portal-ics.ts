...
router.get("/ics", async (_req, res) => {
  ...
  res.setHeader("Content-Disposition", "attachment; filename=spd-fraktion-kl.ics");
  return res.send(toIcs(events));
});

router.get("/calendar/ics", (_req, res) => {
  return res.redirect("/api/ics");
});
