...
function isPatrickLucaTask(task: FraktionTask) { return (task.description ?? "").includes("[Patrick→Luca]") || (task.description ?? "").includes("[Patrick-Luca]"); }
...
function progressPercent(status: string | null | undefined) { const index = boardStatus.indexOf(status || "offen"); return index < 0 ? 8 : Math.max(8, Math.round(((index + 1) / boardStatus.length) * 100)); }
...
export default function Home() {
  ...
  const generalTasks = useMemo(() => tasks.filter(task => !isPatrickLucaTask(task)), [tasks]);
  const patrickLucaTasks = useMemo(() => allowWorkOrders ? tasks.filter(isPatrickLucaTask) : [], [tasks, allowWorkOrders]);
...
function PatrickLucaBoard({ tasks, onStatus, compact = false }: { tasks: FraktionTask[]; onStatus: (task: FraktionTask, status: string) => Promise<void>; compact?: boolean }) { const visibleTasks = compact ? tasks.filter(t => t.status !== "erledigt").slice(0, 4) : tasks; if (!visibleTasks.length) return <p className="muted">Noch keine Patrick-Luca-Aufträge.</p>; return <div className={classNames("monday-board", compact && "compact")}><div className="monday-group-title"><span>{compact ? "Aktive Aufträge" : "Gemeinsame Aufträge"}</span><span>{visibleTasks.length}</span></div><div className="monday-header"><span>Auftrag</span><span>Person</span><span>Status</span><span>Fortschritt</span><span>Fälligkeit</span><span>Priorität</span></div>{visibleTasks.map(task => <WorkOrderRow key={task.id} task={task} onStatus={onStatus} />)}</div>; }
function WorkOrderRow({ task, onStatus }: { task: FraktionTask; onStatus: (task: FraktionTask, status: string) => Promise<void> }) { const current = task.status || "offen"; return <div className="monday-row"><div className="monday-task"><strong>{task.title}</strong>{cleanTaskDescription(task.description) && <small>{cleanTaskDescription(task.description)}</small>}</div><div className="monday-person"><span className="person-dot">LH</span><span>Luca</span></div><details className="status-dropdown"><summary className={`status-cell status-${current}`}>{taskLabel[current] ?? current}</summary><div className="status-menu">{boardStatus.map(status => <button key={status} onClick={() => onStatus(task, status)}>{taskLabel[status]}</button>)}</div></details><div className="progress-cell"><span style={{ width: `${progressPercent(current)}%` }} /></div><div className="due-cell">{dateOnly(task.due_date)}</div><div className="priority-stars" title={task.priority}>{priorityStars(task.priority)}</div></div>; }
